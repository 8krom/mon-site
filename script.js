...
function afficherVideos() {
    ...
    videos.forEach(function(video) {
        ...
        v.preload = "none";          // ne télécharge la vidéo qu'au survol
        v.poster = "images/" + video.poster;  // une vignette légère à la place
...
function afficherMessageConnexion() {
    const dropdown = document.getElementById("panier-dropdown");
    let msg = document.getElementById("msg-connexion");
    if (!msg) {
        msg = document.createElement("div");
        msg.id = "msg-connexion";
        msg.style.cssText = "margin-top:12px; font-size:11px; line-height:1.7; letter-spacing:1px; color:#e8e8e8; border-top:1px solid #333333; padding-top:12px;";
        dropdown.appendChild(msg);
    }
    msg.innerHTML = "Pour passer commande, tu dois d'abord rejoindre VolkCorporation.<br><a href='index.html' style='color:#ec911a; text-decoration:underline; cursor:pointer;'>→ Rejoindre Volkcorp</a>";
}
...
formulaireCommande.addEventListener("submit", function(e) {
    ...
    const commande = document.getElementById("f-commande").value;
    const nom = document.getElementById("f-nom").value;
    const contact = document.getElementById("f-contact").value;

    // Enregistrer dans Firestore
    window.enregistrerCommande(commande + " | Nom : " + nom + " | Contact : " + contact);

    // Envoyer à Formspree sans quitter la page
    const data = new FormData(formulaireCommande);
    fetch(formulaireCommande.action, {
        method: "POST",
        body: data,
    ...
    .then(async response => {
        ...
        console.log("Réponse Formspree :", response.status, result);
        if (!response.ok) throw new Error("Formspree a refusé");
        btnEnvoyer.textContent = "Commande envoyée ✓";
        btnEnvoyer.style.backgroundColor = "#2d6a4f";
        btnEnvoyer.style.borderColor = "#2d6a4f";
        btnEnvoyer.style.color = "#e8e8e8";
        panier.length = 0;
        mettreAJourPanier();
        document.getElementById("confirmation-commande").classList.add("actif");
    })
    .catch((err) => {
        console.log("Erreur attrapée :", err);
        btnEnvoyer.textContent = "Erreur — réessaye";
